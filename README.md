...
2. **CPU app-server:** утилизация в пике до **~7 ядер** (699 %) на 300 юзерах. На машинах с лимитом 4-8 vCPU это и есть «потолок».
3. **Фоновая нагрузка:** после пиковой активности кратковременно поднимается утилизация `cron-worker` (обрабатывает накопившиеся background-job'ы).
4. **Деградация в хвостах:** время отклика отдельных запросов на пике доходит до 20+ с, при этом median остаётся на уровне 19-30 мс, что типично для FPM с очередью.
