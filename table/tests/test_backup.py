...
def test_backup_and_restore_lifecycle(cli):
    ...
    users_before = cli("users", "list", "--filter",
                       "username", "exact", test_user)
    assert test_user in str(users_before)

    # Создание бэкапа
    create_res = cli("backup", "create", "--name",
                     backup_name, "--components", "all")
    ...
    users_deleted = cli("users", "list", "--filter",
                        "username", "exact", test_user)
